...
This repo (koodo-bridge) is a BTECH (Bridgerland Technical College) customization fork of Koodo Reader: a pure web ebook reader (React CRA + Redux), deployed on Cloudflare (`reader.iterverse.net`, migrated from `books.itstem.org` - see ad_labs's `docs/admin-scope.md` for the migration itself). LTI/Canvas embedding was explored early on and has since been removed entirely (BTECH's own call - "unlikely to ever be needed") in favor of the platform auth described below; the shared Iterverse roster/entitlement system covers auth across all Iterverse products, making LTI redundant. It's recoverable from git history if that ever changes.
...
**Mobile responsiveness was largely absent from both the reader and the library page until a dedicated pass added it** - upstream Koodo's web port never really designed either for a phone-width screen (an orphaned, never-referenced `.mobile-warning` CSS class in `manager.css` suggests upstream originally gated narrow screens behind a "please use a bigger screen" message that got lost somewhere, rather than ever making the real UI responsive). The library sidebar (`containers/sidebar/`) now has two independent states that share the same hamburger icon: `isCollapsed` (a persisted desktop preference toggling the full sidebar vs. a narrow icon rail - unchanged) vs. `isMobileMenuOpen` (new, session-only, hidden-by-default overlay drawer below a 768px `window.innerWidth` breakpoint, opened via a *separate*, always-visible `.mobile-menu-toggle` button rendered outside the drawer itself - the drawer's own internal hamburger is unreachable while closed, so don't remove that outer button). The reader's operation/progress bars, side panels, and settings dialog got matching breakpoints. **Watch out for CSS media-query ordering**: a `@media` block placed *before* the base (non-media) rule for the same selector loses the cascade tie regardless of which one's condition actually matches at runtime - this silently no-op'd the first pass at the sidebar's mobile CSS. Keep responsive overrides after every base rule they touch (typically at the end of the file).

**The login page (`src/pages/login/`) got the same mobile-responsiveness treatment, separately from the reader/library pass above** - it was built as a fixed 60/40 flex split (cover photo / sign-in content column) with several pixel-width children (the title's 40px side margins, the sync/option boxes' 260px width) sized for a desktop-width ~40%-of-viewport column; on a phone that column is only ~150px wide, so text and the sign-in button overflowed past the screen edge. Below 700px the two halves now stack vertically instead of splitting side by side. **The BTECH wordmark (`logo-horizontal.png`, 707x261) and the square "b" mark (`logo-mark.png`, 512x512) are both rendered on every login step, with CSS - not JS/device-detection - picking which one is visible** (`.login-logo-img`/`.login-logo-mark-img`, toggled by the same 700px breakpoint): the wide lockup doesn't fit legibly in the mobile layout's short cover banner no matter how it's resized, so don't "clean up" the apparently-duplicate second `<img>` in `Login.renderLogo()` - it's the mobile fallback, not dead code.

**The reader's swipe-to-turn-page gesture is gated by a config key named `isTouch` - do not confuse this with the *different*, same-named `isTouch` React state in `pages/reader/component.tsx`.** The actual gesture handler lives in `src/utils/reader/mouseEvent.ts`'s `bindHtmlEvent` (HammerJS pan recognizer, our own code, not the closed-source engine) and only attaches when `ConfigService.getReaderConfig("isTouch") === "yes"` - a manual, persisted Settings > Reading toggle that nothing ever auto-detected. `reader/component.tsx`'s own `isTouch` *state* only controls that component's edge-hover panel-reveal behavior and reads/writes nothing that `mouseEvent.ts` sees - the two collided by naming coincidence, not design, and changing one silently does nothing for the other. Net effect: swipe-to-turn-page never worked out of the box on a real phone, for this fork's entire history, until `bindHtmlEvent`'s check was extended to `|| isMobileRenderDevice()`. If touch interaction ever seems broken again, check this gate first.

**The reading engine's `isMobile` option (passed via `BookHelper.getRendition(...)`, e.g. in `containers/viewer/component.tsx`) is hardcoded `"no"` everywhere, deliberately.** It's a real, engine-recognized flag (affects scroll-behavior timing, animation delays, image handling per the engine's own minified logic) that had never been turned on in this fork's history. Wiring it to real device detection broke page scrolling on an actual phone the one time it was tried - since the engine is an unreadable/unrebuildable vendored artifact (see below), there's no way to audit or fix whatever that path does wrong, so it's left off. Don't re-enable this without access to the real kookit source and a way to test on a physical device.

**`src/utils/reader/styleUtil.ts`'s `getLegibilityCss()` exists because the engine's own forced-text-color rule is unreliable across themes.** `StyleHelper.getDefaultCss` only adds `!important` to its text-color override for one exact hardcoded dark preset, an explicit "Overwrite the default text style" toggle, or a "night" app skin - any other active background/theme leaves a book's own CSS free to win, potentially producing illegible text. Since the engine can't be patched directly, `getLegibilityCss()` layers an app-level `<style>` tag (appended after the engine's own, before any admin/user custom book CSS) that generalizes the same override to every active theme, and separately forces legible text inside solid-fill highlight spans (`.kookit-highlight-text`, always pale pastel backgrounds regardless of page theme). Relatedly, `getContrastRatio`/`pickReadableCounterColor` in `utils/common.ts` auto-correct a background/text color pair in the theme picker (`readerSettings/themeList/component.tsx`) when a user's custom color choice would otherwise be unreadable.
...
│       ├── reader/         # Reader logic (highlightUtil, mouseEvent (touch/swipe/keyboard gestures), noteUtil, styleUtil, ttsUtil, themeUtil, etc.)
│       ├── request/        # HTTP requests
│       └── storage/        # Storage services (databaseService, syncService)
└── scripts/                # i18n tooling (extract-untranslated, merge-translations)
```
